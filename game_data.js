...
const gameData = {
  ...
  // Fungsi untuk menentukan level kesesuaian karir
  getFitLevel: function(averageScore) {
    if (averageScore >= this.fitLevels.excellent.threshold) {
      return this.fitLevels.excellent;
    } else if (averageScore >= this.fitLevels.good.threshold) {
      return this.fitLevels.good;
    } else if (averageScore >= this.fitLevels.moderate.threshold) {
      return this.fitLevels.moderate;
    } else {
      return this.fitLevels.low;
    }
  },

  // Fungsi untuk mendapatkan random event
  getRandomEvent: function(currentPath) {
    // 30% chance random event muncul
    if (Math.random() > 0.3) {
      return null;
    }

    // Filter events yang applicable untuk path ini
    const applicableEvents = this.randomEvents.filter(
      event => event.applicablePaths.includes(currentPath)
    );
    ...
    const totalProbability = applicableEvents.reduce(
      (sum, event) => sum + event.probability, 0
    );
    
    let random = Math.random() * totalProbability;
    
    for (let event of applicableEvents) {
  ...
  // Fungsi untuk generate rekomendasi karir berdasarkan scoring
  generateRecommendation: function(scores, careerPath) {
    const avgScore = this.calculateAverageScore(scores);
    const fitLevel = this.getFitLevel(avgScore);
    const pathInfo = this.careerPaths[careerPath];

    let recommendation = {
      careerPath: pathInfo.label,
      averageScore: avgScore,
      fitLevel: fitLevel.label,
      fitColor: fitLevel.color,
      message: fitLevel.message,
      scores: scores,
      strengths: [],
      improvements: [],
      advice: ""
    ...
    // Identifikasi kekuatan (dimensi dengan skor tinggi)
    for (let [dimension, score] of Object.entries(scores)) {
      if (score >= 70) {
        recommendation.strengths.push({
          dimension: this.scores[dimension].label,
          score: score
        });
      } else if (score < 50) {
    ...
    // Generate advice berdasarkan fit level
    if (avgScore >= 80) {
      recommendation.advice = `Jalur karir ${pathInfo.label} sangat cocok untukmu! Pertahankan passion dan terus kembangkan diri di jalur ini.`;
    } else if (avgScore >= 65) {
      recommendation.advice = `Jalur karir ${pathInfo.label} cukup sesuai. Fokus pada area yang perlu improvement untuk hasil maksimal.`;
    } else if (avgScore >= 50) {
      recommendation.advice = `Jalur karir ${pathInfo.label} bisa jadi pilihan, tapi pertimbangkan untuk eksplorasi lebih dalam sebelum commit penuh.`;
    } else {
      recommendation.advice = `Jalur karir ${pathInfo.label} mungkin kurang cocok. Pertimbangkan jalur karir alternatif yang lebih sesuai dengan profil dan aspirasimu.`;
    }
